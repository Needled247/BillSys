...
<%
    ...
    while (it.hasNext()){
        ...
        userIdText = view.getUserId();
        mobileText = view.getMobile();
        phoneIpText = view.getIpAdd();
        vlanText = view.getVlan();
        longNumText = view.getLongNum();
        shortNumText = view.getShortNum();
        itimeText = view.getUpTime().toString();
        tblName = view.getTbl();
        isHandle = view.getHandle();
        gate = view.getGate();
        if(isHandle.equals("1")||isHandle=="1"){
            HandleText = "已处理";
        }
        else {
            HandleText = "未处理";
...
<div class="box-content">
    ...
    <fieldset>
        ...
        <input type="hidden" id="tblName" value="<%=tblName%>">
        <div class="form-actions">
            <button type="button" class="btn btn-primary" onclick="EditFreeNumInfo()">保存修改</button>
            <button type="button" class="btn btn-warning" onclick="initNum()">号码初始化</button>
            <button type="button" class="btn btn-danger" onclick="return delFreeNumInfo()">删除号码</button>
        </div>
    </fieldset>
    </form>
</div>
