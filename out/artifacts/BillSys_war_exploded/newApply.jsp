...
<div class="box-header well" data-original-title="">
    <h2><i class="icon-user"></i> 查看新申请</h2>
</div>
...
<tbody role="alert" aria-live="polite" aria-relevant="all">
    ...
    <tr class="even">
        <%}%>
        <td class="  sorting_1"><%=view.getLongNum()%></td>
        <td class="center "><%=view.getUserId()%></td>
        <td class="center "><%=view.getMobile()%></td>
        <td class="center "><%=newTime%></td>
        <!--span class="label label-warning">warn</span-->
        <td class="center "><%=new BillSysTool().tblName2departmetName(view.getTbl())%></td>
        <td class="center ">
            <a class="btn btn-success" href="ViewApply.jsp?pnum=<%=view.getLongNum()%>&userid=<%=view.getUserId()%>&mobile=<%=view.getMobile()%>">
                <i class="icon-zoom-in icon-white"></i>查看/编辑</a>
            <a class="btn btn-danger" href="DetailEdit.action?editType=page_init&finalNum=<%=view.getLongNum()%>&finalUser=<%=view.getUserId()%>&ipadd=<%=view.getIpAdd()%>">
